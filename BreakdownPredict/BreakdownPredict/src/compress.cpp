...
HISTOGRAM* DecompressHist(void* H, int Size)                                    //алгортм запаковки
{
	int Size0, Size1;
	void* H1;
	HISTOGRAM* H2;
...
int DefineVersion(HANDLE hFile, char* FileHeader)                               //Определение версии файла
{
	bool ValidRead;
	bool HeaderRead;
	char FileHeader0[] = "ANALIZER1.0";
	char FileHeader1[] = "ANALIZER1.1";
	...
	ValidRead = ReadFile(hFile, FileHeader, sizeof(FileHeader2), &NBytes, NULL);

	if (strcmp(FileHeader, FileHeader0) == 0) {
		HeaderRead = TRUE;
		return 0;
	}
	else if (strcmp(FileHeader, FileHeader1) == 0) {
		HeaderRead = TRUE;
		return 1;
	}
	else if (strcmp(FileHeader, FileHeader2) == 0) {
		HeaderRead = TRUE;
		return 2;
	}
	else {
...
#define InBuff ((unsigned char *)inbuff)
	...
	do {
		if (i == SizeIn - 1) Repeat = false;
		else Repeat = InBuff[i] == InBuff[i + 1];

		if (Repeat) {
			Counter = 2;
			i += 2;
			while ((i < SizeIn) && (InBuff[i] == InBuff[i - 1]) && (Counter < 127)) {
				i++;
				Counter++;
			}
			NBytes += 2;
...
void CreateCode(unsigned char* Table, CODE* Code)
{
	...
	for (i = 0; i < 256; i++) {
		...
		if (Table[i] != 255) {
			...
			do {
				if (Mask[Vertex] == 512) Mask[Vertex] = PrevVertex;
...
void ChangeByteOrder(void* data, int DataSize, int Level, bool Direction)
{
	...
	for (i = 0; i < DataSize; i++) Data[i] = Data1[i];
