...
void Pack(wchar_t FullFileNameOut[256], HISTOGRAM* Data[100], char FileHeader[12], int Count) // упаковка данных из Data в .SHT файл
{
	...
	hFileOut = CreateFileW(FullFileNameOut, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
	printf("\n");
...
double getY(HISTOGRAM* H, int i)
{
	...
	{
	case 0:
		return H->Data[i] * H->Delta + H->Umin;
	case 1:
